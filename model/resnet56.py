...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False, groups=1,
        ...
        if self.tier == 3 or self.tier == 2 or self.tier == 1:
             self.layer2 = self._make_layer(block, 16, layers[0])
        if self.tier == 2 or self.tier == 1: 
             self.layer3 = self._make_layer(block, 32, layers[1], stride=2)
        if self.tier == 1:
             self.layer4 = self._make_layer(block, 32, layers[2], stride=1)
...
class ResNet_server(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False, groups=1,
        ...
        if self.tier == 5:
            self.inplanes = 16
        elif self.tier == 4 or self.tier == 3:
            self.inplanes = 64
        elif self.tier == 2:
        ...
        if self.tier == 5 or self.tier == 4 or self.tier == 3:
