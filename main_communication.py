...
epsilon = 0.5  # Chosen privacy budget
noise_multiplier = 1.0  # Scale of the noise
...
def add_args(parser):
    ...
    parser.add_argument('--whether_pretrained', default=0, type=int)
    parser.add_argument('--whether_differential_privacy', default=0,
                        type=int)  # 2 my code, 1 pydp   # opacus can not work with batch normalizeation layer
    parser.add_argument('--dp_epsilon', default=0.5, type=int)

    # communication efficiency
    parser.add_argument('--whether_svd_feature_map', default=True, type=bool)
    parser.add_argument('--svd_rank', default=20, type=int)
    ...
    parser.add_argument('--test_before_train', type=int, default=0, metavar='N',
    ...
    parser.add_argument('--computation_speeds_list', type=str, default=[27, 21, 12, 6.5, 3.2],
...
computation_speeds = [0.4, 1, 1.2, 0.2, 0.8]
...
if args.whether_FedAVG_base:
    ...
    client_type_percent = [0.0, 0.5, 0.0, 0.0, 0.5, 0.0, 0.0]
    tier = 1
...
net_speed_list = np.array([100, 200, 500]) * 1024000 ** 2
net_speed_weights = [0.5, 0.25, 0.25]
...
def add_laplacian_noise(model_state_dict, epsilon, sensitivity):
    """Adds Laplacian noise to model weights.

    Args:
        weights: The model weights (tensor).
        epsilon (float): The privacy budget (epsilon).
        sensitivity (float): The sensitivity of the weights.

    Returns:
        The noisy weights (tensor).
    """

    noisy_state_dict = model_state_dict.copy()  # Avoid modifying the original
    for name, param in noisy_state_dict.items():
        if param.requires_grad or True:  # Only add noise to trainable parameters
            noise_scale = epsilon * sensitivity
            noise = torch.tensor(
                [torch.distributions.Laplace(loc=0.0, scale=noise_scale).sample()
...
if torch.cuda.device_count() > 1:
    ...
    net_glob_server = nn.DataParallel(net_glob_server,
                                      device_ids=list(range(torch.cuda.device_count())))  # to use the multiple GPUs
    for i in range(1, num_tiers + 1):
...
def criterion_KL(outputs, teacher_outputs):
    ...
    criterion_KL = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(outputs / T, dim=1),
                                                       ...
                                                       F.softmax(teacher_outputs / T, dim=1) + 10 ** (-7)) * (
                               args.KD_beta * T * T)
    return criterion_KL
...
def FedAvg(w):
    ...
    if args.whether_differential_privacy == 1:

        for k in w_avg.keys():
            # values = [w[i][k] for i in range(1, len(w))]  # Extract values for key 'k' from all clients
            values = [item for sublist in w[1:] for item in sublist[k].tolist()]
            dp_mechanism = BoundedMean(epsilon=args.dp_epsilon)  # Set the privacy parameter epsilon

            # Compute the differentially private mean using PyDP's BoundedMean
            noisy_mean = dp_mechanism.quick_result(values)
            w_avg[k] += noisy_mean

        # Normalize the averaged weights (divide by the number of clients)
        num_clients = len(w) - 1  # Excluding the first client used for finding the minimum length
        for k in w_avg.keys():
...
optimizer_server_glob = torch.optim.Adam(net_server.parameters(), lr=lr, weight_decay=args.wd,
                                         amsgrad=True)  # from fedgkt code
...
def train_server(fx_client, y, l_epoch_count, l_epoch, idx, len_batch, extracted_features):
    ...
    if args.optimizer == "Adam":  # and False:
        optimizer_server = torch.optim.Adam(net_server.parameters(), lr=lr, weight_decay=args.wd,
                                            amsgrad=True)  # from fedgkt code
    ...
    elif args.whether_svd_feature_map:
    ...
    if count1 == len_batch:
        ...
        wandb.log({"Client{}_Training_Time_in_Server".format(idx): time_train_server_train, "epoch": l_epoch_count},
                  commit=False)
        ...
        if len(idx_collect) == m:  # federation after evfery epoch not when all clients complete thier process like splitfed
            ...
            if not whether_GKT_local_loss:
                if not args.whether_aggregated_federation == 1:
                    w_glob_server = FedAvg(
                        w_locals_server)  # check with w_locals_server[1]['fc.bias'] and w_locals_server_tier[5][0]['fc.bias']
                    if not whether_multi_tier:
                        ...
                        if inter_tier_fedavg:
                            w_glob_server_tier = multi_fedavg(w_glob_server_tier, num_tiers, client_number_tier,
                                                              client_tier, idx_collect, agent='server')
...
def evaluate_server(fx_client, y, idx, len_batch, ell):
    ...
    with torch.no_grad():
        ...
        if count2 == len_batch:
            ...
            prGreen('Update Model Test =>                   \tAcc: {:.3f} \tLoss: {:.4f}'.format(acc_avg_test,
                                                                                                 loss_avg_test))
...
class Client(object):
    ...
    def train(self, net):
        ...
        if args.optimizer == "Adam":
            optimizer_client = torch.optim.Adam(net.parameters(), lr=lr, weight_decay=args.wd,
                                                amsgrad=True)  # from fedgkt code
        elif args.optimizer == "SGD":
        ...
        # privacy_engine.attach(optimizer_client)
        ...
        for iter in range(self.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                if args.whether_FedAVG_base or client_tier[idx] == 0:
                    ...
                    compression_ratio = 1
                    if args.whether_svd_feature_map:
                        compressed_fx = []
                        # one option is to svd across all channels
                        for client_feature in fx:  # Iterate over batch
                            flattened = client_feature.reshape(len(fx[0]), -1).detach().cpu().numpy()  # Flatten to 256 x 64
                            u, s, vh = randomized_svd(flattened, n_components=min(args.svd_rank, len(flattened)), random_state=42)
                            compressed = np.dot(u, np.dot(np.diag(s), vh))
                            compressed_fx.append(
                                torch.tensor(compressed).reshape(client_feature.size(0), client_feature.size(1), -1))
                            # compressed_fx.append(compressed.reshape(256, 8, 8))
                        fx = torch.stack(compressed_fx, dim=0)
                        # torch.mean((fx - torch.stack(compressed_fx, dim=0)) ** 2).item()
                        compression_ratio = (u.size + s.size + vh.size) / flattened.size
                        # assert fx.requires_grad
                        fx.requires_grad_()
                        # compression_ratio =
                    ...
                    if whether_distillation_on_clients:
                        ...
                        train_server(client_fx, labels, iter, self.local_ep, self.idx, len_batch, _)
                    else:
                    ...
                    if whether_local_loss:
                        ...
                        if args.whether_differential_privacy == 2:
                            ...
                            #         noise = torch.randn_like(param.grad) * noise_multiplier * epsilon
                            #         param.grad += noise

                            #         # noise = torch.tensor(np.random.laplace(0, noise_multiplier, param.grad.shape))
                            #         # param.grad += noise

                            noisy_weights = add_laplacian_noise(net.state_dict(), epsilon=0.5, sensitivity=1.0)
                    ...
                    if whether_GKT_local_loss:
                        data_transmited_sl_client += (sys.getsizeof(client_fx.storage()) +
                                                      sys.getsizeof(extracted_features.storage()) + sys.getsizeof(
                                    labels.storage()) +
                                                      sys.getsizeof(dfx.storage()))
                    elif whether_local_loss:
                        data_transmited_sl_client += ((sys.getsizeof(client_fx.storage()) * compression_ratio )+
        ...
        if args.whether_FedAVG_base or client_tier[idx] == 0:
            return net.state_dict(), time_client, data_transmited_sl_client, sum(epoch_loss) / len(epoch_loss), sum(
                epoch_acc) / len(epoch_acc)
...
for i in range(0, num_users):  # one optimizer for every tier/ client
    if args.optimizer == "Adam":
        optimizer_client_tier[i] = torch.optim.Adam(net_glob_client_tier[client_tier[i]].parameters(), lr=lr,
                                                    weight_decay=args.wd, amsgrad=True)  # from fedgkt code
    elif args.optimizer == "SGD":
        optimizer_client_tier[i] = torch.optim.SGD(net_glob_client_tier[client_tier[i]].parameters(), lr=lr,
                                                   momentum=0.9,
                                                   nesterov=True,
                                                   weight_decay=args.wd)
...
for iter in range(epochs):
    ...
    args.KD_beta = args.KD_beta_init + (-args.KD_beta_init) * math.exp(
        -1. * args.KD_increase_factor * iter / args.rounds)
    ...
    for idx in idxs_users:
        wandb.log({"Client{}_Tier".format(idx): num_tiers - client_tier[idx] + 1, "epoch": iter},
                  commit=False)  # check to tier be in accordance of the paper order
        ...
        if whether_multi_tier:
            ...
            w_glob_client_tier[client_tier[idx]] = net_glob_client_tier[
                client_tier[idx]].state_dict()  # this is weight of the model
        if args.dataset == "HAM10000":
        ...
        # Training ------------------
        if args.whether_FedAVG_base or client_tier[idx] == 0:
            [w_client, duration, data_transmited_sl_client, loss_train, acc_train] = local.train(
        ...
        wandb.log({"Client{}_Data_Transmission(MB)".format(idx): data_transmitted_client / 1024 ** 2, "epoch": iter},
                  commit=False)
        wandb.log({"Client{}_Data_Transmission(MB)_Model_Parameters".format(idx): data_transmited_fl_client / 1024 ** 2,
                   "epoch": iter}, commit=False)
        wandb.log(
            {"Client{}_Data_Transmission(MB)_Intermediate_data".format(idx): data_transmited_sl_client / 1024 ** 2,
             "epoch": iter}, commit=False)
    ...
    simulated_delay[
        simulated_delay == 0] = np.nan  # convert zeros to nan, for when some clients not involved in the epoch
    simulated_delay_historical_df = pd.concat([simulated_delay_historical_df, pd.DataFrame(simulated_delay).T],
                                              ignore_index=True)
    ...
    if not args.whether_FedAVG_base:
        [client_tier, client_epoch, avg_tier_time_list, max_time_list, client_times] = tier_scheduler(
            client_tier_all[:], simulated_delay_historical_df,
    ...
    if args.dataset != "HAM10000":
        client_sample = calculate_client_samples(train_data_local_num_dict, idxs_users,
                                                 args.dataset)  # same order as appended weights
    else:
        client_sample = calculate_client_samples(train_data_local_num_dict, idxs_users,
                                                 args.dataset)  # same order as appended weights
    ...
    if args.whether_FedAVG_base:
        ...
        if args.whether_aggregated_federation == 1:
            if args.wheter_agg_tiers_on_server == 1:  # this part help to have between tier aggregation with last update of clients
                for n in list(idxs_users)[::-1]:
                    for m in [k for k, v in client_tier_all[-2].items() if v == client_tier_all[-2][n]]:
                        w_locals_tier[list(idxs_users).index(m)] = copy.deepcopy(
                            w_locals_tier[list(idxs_users).index(n)])
            if args.whether_local_loss_v2:
                w_glob = aggregated_fedavg(w_locals_tier, w_locals_client, num_tiers, num_users, whether_local_loss,
                                           client_sample, idxs_users,
                                           local_v2=args.whether_local_loss_v2)  # w_locals_tier is for server-side
            else:
                w_glob = aggregated_fedavg(w_locals_tier, w_locals_client, num_tiers, num_users, whether_local_loss,
                                           client_sample, idxs_users)  # w_locals_tier is for server-side

            for t in range(1, num_tiers + 1):
                if whether_federation_at_clients:
                    for k in w_glob_client_tier[t].keys():
                        if k in w_glob_server_tier[
                            t].keys():  # This is local updading  // another method can be updating and supoose its similar to global model
            ...
            for k in w_client:  # this is sum of model size of all clients server sent to each clients
                data_transmited_fl = data_transmited_fl + len(w_locals_client) * sys.getsizeof(
                    w_glob_client[k].storage())
            ...
            if inter_tier_fedavg:
                w_glob_client_tier = multi_fedavg(w_glob_client_tier, num_tiers, client_number_tier, client_tier,
                                                  idx_collect, agent='client')
            for t in range(1, num_tiers + 1):
                net_glob_client_tier[t].load_state_dict(w_glob_client_tier[t])

            for t in range(1, num_tiers + 1):
                for k in w_glob_client_tier[t]:  # this is sum of model size of all clients server sent to each clients
                    data_transmited_fl = data_transmited_fl + client_number_tier[t - 1] * sys.getsizeof(
                        w_glob_client_tier[t][k].storage())
